...
class RADIO:
    def __init__(self,
        ...
        if spi_hardware:
            ...
            if ESP32:
                # Use VSPI (SPI(2)) on ESP32 with custom mapping
                sck_pin   = Pin(gpio['sck'], Pin.OUT)
                mosi_pin  = Pin(gpio['mosi'], Pin.OUT)
                miso_pin  = Pin(gpio['miso'], Pin.IN)
                self.spi = SPI(2, baudrate=spi_baudrate, polarity=0, phase=0,
